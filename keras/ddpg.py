...
from keras.layers import Dense, Input, Add, Dropout, BatchNormalization, Activation, Lambda
...
class ActorNetwork():
    ...
    def create_actor_network(self):
        ...
        output = Dense(400)(observation_in)
        output = BatchNormalization()(output)
        output = Activation('relu')(output)
        output = Dense(300)(output)
        output = BatchNormalization()(output)
        output = Activation('relu')(output)
        output = Dense(self.action_size, activation='tanh', kernel_initializer=RandomUniform(-0.003, 0.003))(output)
...
class CriticNetwork():
    ...
    def create_critic_network(self):
        observation_in = Input(shape=(self.feature_size,))
        observation_output = Dense(256)(observation_in)
        observation_output = BatchNormalization()(observation_output)
        observation_output = Activation('relu')(observation_output)
        observation_output = Dense(256, activation='linear')(observation_output)

        action_in = Input(shape=(self.action_size,))
        action_output = Dense(256, activation='linear')(action_in)
